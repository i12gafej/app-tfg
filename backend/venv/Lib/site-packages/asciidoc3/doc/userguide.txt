...
Visit the https://asciidoc3.org[homepage] of the project to find additional information.

If you're new to AsciiDoc3 read the Introduction / Getting Started section and the
<<X6,Introduction / Getting Started>> section and take a look at the example AsciiDoc3 (*.txt)
source files in the distribution directories './doc/' and './tests/data'. They 
may be useful as 'templates' to learn more about the features of AsciiDoc3.


== Introduction / Getting Started
[[X6]]
AsciiDoc3 is a plain text human readable/writable document format that
can be translated to DocBook (v4.5/v5.1) or HTML using the asciidoc3 command.
You can then either use asciidoc3 generated HTML directly or run
asciidoc3 DocBook output through your favorite DocBook toolchain or
use the AsciiDoc3 a2x3 toolchain wrapper to produce PDF, EPUB, DVI,
LaTeX, PostScript, man page, (slidy) HTML and text formats.

The AsciiDoc3 format is a useful presentation format in its own right:
AsciiDoc3 markup is simple, intuitive and as such is easily proofed and
edited.

AsciiDoc3 is light weight: it consists of a single Python script and a
bunch of configuration files. Apart from asciidoc3 and a Python
interpreter, no other programs are required to convert AsciiDoc3 text
files to DocBook or HTML. See <<X11,Example AsciiDoc3 Documents>>
below.

Text markup conventions tend to be a matter of (often strong) personal
preference: if the default syntax is not to your liking you can define
your own by editing the text based asciidoc3 configuration files.
You can also create configuration files to translate AsciiDoc3
documents to almost any SGML/XML markup.

AsciiDoc3 comes with a set of configuration files to translate
AsciiDoc3 articles, books and man pages to HTML or DocBook backend
formats.

.My AsciiDoc Itch (Stuart Rackham - AsciiDoc v2)
...
.My AsciiDoc Itch (Berthold Gehrke - AsciiDoc3)
I came in contact with AsciiDoc when working for a publishing company
in the 2000s. Since then I use this awful software almost every day to
support my profession as a software developer. Since there seems to be
no Python3 version, I began porting it by myself - and here's the result.




Installing AsciiDoc3
~~~~~~~~~~~~~~~~~~~
See the infos given in file INSTALL and at https://asciidoc3.org/install.html
You can download the tarball, zip-file, deb/rpm packages or using pip or clone the gitlab-repo. AsciiDoc3 does work on Windows.


[[X11]]
Example AsciiDoc3 Documents
~~~~~~~~~~~~~~~~~~~~~~~~~~
See the infos given in file quickstart and at asciidoc3.org. Take a look at directory 
'./doc' in the distributed files to find some more txt-files to explore the power of AsciiDoc3. +
Even more tests are found in './tests/data'. +
When this directory is not provided in your tarball, you may find it in the
repo https://gitlab.com/asciidoc3/asciidoc3/tree/master/tests/data


AsciiDoc3 Document Types
-----------------------
There are three types of AsciiDoc3 documents: article, book and
manpage. All document types share the same AsciiDoc3 format with some
minor variations. If you are familiar with DocBook you will have
noticed that AsciiDoc3 document types correspond to the same-named
DocBook document types.

Use the asciidoc3 `-d` (`--doctype`) option to specify the AsciiDoc3
document type -- the default document type is 'article'.

By convention the .txt file extension is used for AsciiDoc3 document
source files. If you make use of the 'Pelican Static Site Generator, Powered by Python'
https://getpelican.com' (as we do in our blog, see
https://asciidoc3.org/blog/pelican-plugin-for-asciidoc3-available.html) the extension is '.ad3'


=== Encoding

NOTE: To set the encoding of your input and output make use of the attributes
'input-encoding' and 'output-encoding', respectively (default 'utf-8'). These
attributes have to be set in the 'asciidoc3.conf' or in the command line!
Do not set them in your input-file, this will be ignored.



article
~~~~~~~
Used for short documents, articles and general documentation; probably the very
most documents in the real world are an 'article'. See the
AsciiDoc3 distribution `./doc/article.txt` example.

AsciiDoc3 defines standard DocBook article frontmatter and backmatter
...
AsciiDoc3 defines standard DocBook book frontmatter and backmatter
<<X93,section markup templates>> (appendix, dedication, preface,
bibliography, glossary, index, colophon).

.Example book documents
Book::
  The `./doc/book.txt` file in the AsciiDoc3 distribution.

Multi-part book::
  The `./doc/book-multi.txt` file in the AsciiDoc3 distribution.

manpage
~~~~~~~
Used to generate roff format UNIX manual pages.  AsciiDoc3 manpage
documents observe special header title and section naming conventions
-- see the <<X1,Manpage Documents>> section for details.

AsciiDoc3 defines the 'synopsis' <<X93,section markup template>> to
generate the DocBook `refsynopsisdiv` section.

See also the asciidoc3 man page source (`./doc/asciidoc3.1.txt`) from
the AsciiDoc3 distribution.


[[X5]]
AsciiDoc3 Backends
-----------------
The asciidoc3 command translates an AsciiDoc3 formatted file to the
backend format specified by the `-b` (`--backend`) command-line
option. AsciiDoc3 itself has little intrinsic knowledge of backend
...
docbook51::
  Outputs DocBook XML 5.1 markup.

html4::
  This backend generates plain HTML 4.01 Transitional markup (standard until 2014). 
...
html5::
  ...
  the 'xhtml11' backend (standard since 2015).

slidy::
  Use this backend to generate self-contained
  http://www.w3.org/Talks/Tools/Slidy2/[Slidy] HTML slideshows for
  your web browser from AsciiDoc3 documents. The Slidy backend is
  documented in the distribution `doc/slidy.txt` file and
  {website}slidy.html[online].

latex::
  Experimental LaTeX backend. // work in progress

Backend Aliases
~~~~~~~~~~~~~~~
Backend aliases are alternative names for AsciiDoc3 backends.  AsciiDoc3
comes with two backend aliases: 'html' (aliased to 'xhtml11') and
'docbook' (aliased to 'docbook45').

You can assign (or reassign) backend aliases by setting an AsciiDoc3
attribute named like `backend-alias-<alias>` to an AsciiDoc3 backend
name. For example, the following backend alias attribute definitions
appear in the `[attributes]` section of the global `asciidoc3.conf`
configuration file:
...
[TIP]
===============================
If you want to switch permanently to the DocBook51 backend, edit asciidoc3.conf:
uncomment line 'backend-alias-docbook=docbook51' and comment out
'backend-alias-docbook=docbook45'.
To obtain a DocBook51 document just this time and leave the default docbook45 backend:
...
Backend Plugins
~~~~~~~~~~~~~~~
The asciidoc3 `--backend` option is also used to install and manage
...
- Backend plugins <<X27,take precedence>> over built-in backends with
  the same name.
- You can use the `{asciidoc3-confdir}` <<X60, intrinsic attribute>> to
...
- By default backends plugins are installed in
  `$HOME/.asciidoc3/backends/<backend>` where `<backend>` is the
  backend name.


DocBook
-------
AsciiDoc3 generates 'article', 'book' and 'refentry'
http://www.docbook.org/[DocBook] documents (corresponding to the
AsciiDoc3 'article', 'book' and 'manpage' document types).
...
[TIP]
AsciiDoc3 (from version 3.2.0 onwards), at your choice, produces valid DocBook 4.5 or Docbook 5.1 documents. +
See './doc/readme_docbook51.txt' for more information.


[[X12]]
Converting DocBook to other file formats
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
DocBook files are validated, parsed and translated to various
...
chain is to read the DocBook markup (produced by AsciiDoc3) and
...
[[X43]]
a2x3 Toolchain Wrapper
~~~~~~~~~~~~~~~~~~~~~
One of the biggest hurdles for new users is installing, configuring
and using a DocBook XML toolchain. `a2x3` can help -- it's a
toolchain wrapper command that will generate XHTML (chunked and
unchunked), PDF, EPUB, DVI, PS, LaTeX, man page, HTML Help and text
file outputs from an AsciiDoc3 text file.  `a2x3` does all the grunt
work associated with generating and sequencing the toolchain commands
and managing intermediate and output files. `a2x3` also optionally
deploys admonition and navigation icons and a CSS stylesheet. See the
`a2x3` man page for more details. In addition to `asciidoc3` you
...
the AsciiDoc3 `doc/source-highlight-filter.txt` source file. The first
...
HTML generation
~~~~~~~~~~~~~~~
AsciiDoc3 produces nicely styled HTML directly without requiring a
...
- If the DocBook 'lang' attribute is set then things like table of
  ...
  output in the specified language (setting the AsciiDoc3 'lang'
  attribute sets the DocBook 'lang' attribute).

On the other hand, HTML output directly from AsciiDoc3 is much faster,
is easily customized and can be used in situations where there is no
suitable DocBook toolchain.

.The two ways to generate HTML
  $ asciidoc3 -a toc -n -a icons doc/test.txt
  $ a2x3 -f xhtml doc/test.txt
...
- 'dblatex' is easier to install, there's zero configuration
  required and no Java to install -- it just works out of the box.
...
[TIP]
FOP seems to be a little slow when processing large (more than
200 pages *and* a noteworthy amount of tables, lists ...) DocBook51 documents. 
You may go back to DocBook45 or use 'dblatex'.
...
. Convert the HTML Help source (`.hhp` and `.html`) files to HTML Help
  (`.chm`) files using the <<X67,Microsoft HTML Help Compiler>>. (Sorry, not yet tested with AsciiDoc3)


Toolchain components summary
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
AsciiDoc3::
    Converts AsciiDoc3 (`.txt`) files to DocBook XML <<db4551,v4.5/v5.1>> (`.xml`)  files.
...
[[X67]]Microsoft Help Compiler::
  ...
  https://www.microsoft.com/en-us/download/details.aspx?id=21138. (Sorry, not yet tested with AsciiDoc3!)
  

AsciiDoc3 dblatex configuration files
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
The AsciiDoc3 distribution `./dblatex` directory contains
`asciidoc3-dblatex.xsl` (customized XSL parameter settings) and
`asciidoc3-dblatex.sty` (customized LaTeX settings). These are examples
of optional <<X31,dblatex>> output customization and are used by
<<X43,a2x3>>.

AsciiDoc3 DocBook XSL Stylesheets drivers
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
You will have noticed that the distributed HTML and HTML Help
documentation files (for example `./doc/asciidoc3.html`) are not the
...
(`./doc`) directory. These drivers are also used by <<X43,a2x3>>.
...
`chunked.xsl`::
    ...
    $ python3 ../asciidoc3.py -b docbook asciidoc3.txt
    $ xsltproc --nonet ../docbook-xsl/chunked.xsl asciidoc3.xml

`epub.xsl`::
    Used by <<X43,a2x3>> to generate EPUB formatted documents.

`fo.xsl`::
    Generate XSL Formatting Object (`.fo`) files for subsequent PDF
    file generation using FOP. For example:

    $ python3 ../asciidoc3.py -b docbook article.txt
...
`htmlhelp.xsl`::
    ...
    MS Windows from a Cygwin shell prompt (Sorry, not yet tested with AsciiDoc3):

    $ python3 ../asciidoc3.py -b docbook asciidoc3.txt
    $ xsltproc --nonet ../docbook-xsl/htmlhelp.xsl asciidoc3.xml
    $ c:/Program\ Files/HTML\ Help\ Workshop/hhc.exe htmlhelp.hhp

`manpage.xsl`::
    Generate a `roff(1)` format UNIX man page from a DocBook XML
    'refentry' document. This example generates an `asciidoc3.1` man
    page file:

    $ python3 ../asciidoc3.py -d manpage -b docbook asciidoc3.1.txt
    $ xsltproc --nonet ../docbook-xsl/manpage.xsl asciidoc3.1.xml

`xhtml.xsl`::
    Convert a DocBook XML file to a single XHTML file. For example:

    $ python3 ../asciidoc3.py -b docbook asciidoc3.txt
    $ xsltproc --nonet ../docbook-xsl/xhtml.xsl asciidoc3.xml > asciidoc3.html


Generating Plain Text Files
---------------------------
AsciiDoc3 does not have a text backend (for most purposes AsciiDoc3
source text is fine), however you can convert AsciiDoc3 text files to
formatted text using the AsciiDoc3 <<X43,a2x3>> toolchain wrapper
utility.
...
- If the AsciiDoc3 'linkcss' attribute is defined then CSS and
...
- The default locations for CSS and JavaScript files can be changed by
  setting the AsciiDoc3 'stylesdir' and 'scriptsdir' attributes
  respectively.
- The default locations for embedded and linked files differ and are
  ...
  asciidoc3 generates the output document, linked files are loaded
...
- Embedded files are automatically inserted in the output files but
  ...
  AsciiDoc3 <<X27,configuration directories>> to the correct location
...
|`stylesheets` subdirectory in the AsciiDoc3 configuration directory
...
|Absolute or relative to the AsciiDoc3 configuration directory (the
...
|Undefined (default).
|Same directory as the output document.
|`javascripts` subdirectory in the AsciiDoc3 configuration directory
(the directory containing the backend conf file).
...
Themes
~~~~~~
The AsciiDoc3 'theme' attribute is used to select an alternative CSS
stylesheet and to optionally include additional JavaScript code.

- Theme files reside in an AsciiDoc3 <<X27,configuration directory>>
  named `themes/<theme>/` (where `<theme>` is the the theme name set
  by the 'theme' attribute). AsciiDoc3 sets the 'themedir' attribute
  to the theme directory path name.
- The 'theme' attribute can also be set using the asciidoc3
  `--theme` option, the `--theme` option can also be used to manage
  theme <<X101,plugins>>.
- AsciiDoc3 ships with two themes: 'flask' and 'volnitsky' (the default).
- The `<theme>.css` file replaces the default `asciidoc3.css` CSS file.
- The `<theme>.js` file is included in addition to the default
  `asciidoc3.js` JavaScript file.
...
theme=foo`) will cause asciidoc3 to search <<X27,configuration
...
[TIP]
Try the second shipped theme 'flask', some people say it has a 
more modern and lightweight look: +
'asciidoc3 -a toc -n -a icons --theme flask doc/userguide.txt'


Document Structure
------------------
An AsciiDoc3 document consists of a series of <<X8,block elements>>
...
AsciiDoc3 document: documents can range from a single sentence to a
multi-part book.
...
- The header can include <<X18,attribute entries>>, typically
  'doctype', 'lang', 'icons', 'data-uri', 'toc',
  'numbered'.
...
+
--
  * AsciiDoc3 extracts the 'revnumber', 'revdate', and 'author'
...
'firstname' and 'lastname' attributes using the asciidoc3 `-a`
(`--attribute`) command-line option. For example:

  $ asciidoc3 -a revdate=2004/07/27 article.txt
...
The 'title' element in HTML outputs is set to the AsciiDoc3 document
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
AsciiDoc3 has two mechanisms for optionally including additional
...
named like the AsciiDoc3 source file with a `-docinfo` suffix.  For
...
DocBook::
  ...
  `./doc/article-docinfo.xml` example that comes with the AsciiDoc3
  distribution - `./doc/article_docbook51-docinfo.xml` when using the
  DocBook51 backend. The rendering of meta-data elements (or not) is
...
Sections
~~~~~~~~
In addition to the document title (level 0), AsciiDoc3 supports four
section levels: 1 (top) to 4 (bottom).  Section levels are delimited
by section <<X17,titles>>.  Sections are translated using
configuration file <<X93,section markup templates>>. AsciiDoc3
...
AsciiDoc3 configuration files.  Section markup template names are
...
'sect3', 'sect4') AsciiDoc3 has the following templates for
...
^^^^^^^^^^^^^^^^^^^^^^
AsciiDoc3 has a mechanism for mapping predefined section titles
...
name are specified in `[specialsections]` sections in the Asciidoc3
...
matches an AsciiDoc3 document section title then the backend output is
...
of the AsciiDoc3 section title. If `<template>` is blank then any
existing entry with the same `<title>` will be deleted.

.Special section titles vs. explicit template names
*********************************************************************
AsciiDoc3 has two mechanisms for specifying non-default section markup
...
AsciiDoc3 elements c.f.  bibliographic, Q&A and glossary lists.
...
element syntax is defined in the asciidoc3 configuration files.

Here is a list of AsciiDoc3 inline elements in the (default) order in
...
Document Processing
-------------------
The AsciiDoc3 source document is read and processed as follows:
...
When a block element is encountered asciidoc3.py determines the type of
...
New quote types can be defined by editing asciidoc3 configuration
...
positional attribute ('role' attribute) is translated by AsciiDoc3 to
...
AsciiDoc3 generated HTML outputs.  You can also specify multiple class
names separated by spaces.

CSS rules for text color, text background color, text size and text
decorators are included in the distributed AsciiDoc3 CSS files and are
used in conjunction with AsciiDoc3 'xhtml11', 'html5' and 'docbook'
...
rendering is toolchain specific and is not part of the AsciiDoc3
distribution.
...
instruction is handled by <<X43,a2x3>>.
...
<<X43,a2x3>>. Hard page breaks are sometimes handy but as a general
...
`asciidoc-hr` processing instruction is handled by <<X43,a2x3>>.
...
Replacements
~~~~~~~~~~~~
The following replacements are defined in the default AsciiDoc3
configuration:
...
You can also include arbitrary entity references in the AsciiDoc3
source. Examples:
...
[cols="1e,1,5",frame="topbot",options="header"]
|====================================================================
|Name |Backends |Description

|id |html4, html5, xhtml11, docbook45, docbook51 |
Unique identifier typically serve as link targets.
Can also be set by the 'BlockId' element.

|role |html4, html5, xhtml11, docbook45, docbook51 |
Role contains a string used to classify or subclassify an element and
can be applied to AsciiDoc3 block elements.  The AsciiDoc3 'role'
...
you are going the DocBook route then the <<X43,a2x3>> `--no-icons`
and `--icons-dir` options can be used to set the appropriate XSL
Stylesheets parameters.

By default the asciidoc3 HTML backends generate text captions
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
AsciiDoc3 ships with a number of predefined DelimitedBlocks (see the
`asciidoc3.conf` configuration file in the asciidoc3 program
directory):
...
.An Example Sidebar
************************************************
Any AsciiDoc3 SectionBody element (apart from
...
~~~~~~~~~~~~~~~~~~~~~~~~~
AsciiDoc3 comes pre-configured with a 'qanda' style labeled list for generating
...
Glossary Lists
~~~~~~~~~~~~~~
AsciiDoc3 comes pre-configured with a 'glossary' style labeled list for
...
the AsciiDoc3 `./doc` distribution directory.
...
Bibliography Lists
~~~~~~~~~~~~~~~~~~
AsciiDoc3 comes with a predefined 'bibliography' bulleted list style
generating DocBook bibliography entries. Example:

---------------------------------------------------------------------
[bibliography]
.Optional list title (DocBook51: mandatory)
...
Footnotes
---------
The shipped AsciiDoc3 configuration includes three footnote inline
macros:
...
Indexes
-------
The shipped AsciiDoc3 configuration includes the inline macros for
...
NOTE: Define the AsciiDoc3 'icons' attribute (for example using the `-a
...
- Expansion of macro references can normally be escaped by prefixing a
  backslash character (see the AsciiDoc3 'FAQ' for examples of
...
^^^^^^^^^^^^^^^^^^^^^^^^^
Two AsciiDoc3 inline macros are provided for creating hypertext links
within an AsciiDoc3 document. You can use either the standard macro
...
- The AsciiDoc3 'xhtml11' and 'html5' backends display the `<id>`
...
- The optional 'title' attribute provides a title for the image. The
  ...
  visual browsers (AsciiDoc3 HTML outputs only).
...
and CSS, for example:
  ...
  $ asciidoc3 -a data-uri mydocument.txt

[NOTE]
======
- All current popular browsers support data URIs.
...
hardwired into the asciidoc3 program.

- You can escape system macros with a leading backslash character
  (as you can with other macros).
- The syntax and tasks performed by system macros is built into
  asciidoc3 so they don't appear in configuration files.  You can
...
Take a look at the `*.conf` configuration files in the AsciiDoc3
...
AsciiDoc3 block macro syntax `<name>::<target>[<attrlist>]` where:
...
Tables
------
The AsciiDoc3 table syntax looks and behaves like other delimited block
...
~~~~~~~~~~~~~~~~~~~~~~~~
AsciiDoc3 table data can be 'psv', 'dsv' or 'csv' formatted.  The
default table format is 'psv'.

AsciiDoc3 'psv' ('Prefix Separated Values') and 'dsv' ('Delimiter
...
invoked for each cell. The built-in 'asciidoc3' table style is
implemented using a filter.

[[X89]]
.DocBook table widths
**********************************************************************
The AsciiDoc3 docbook backend generates CALS tables. CALS tables do not
...
table width as a percentage of the available width. AsciiDoc3 emits
these processing instructions if the 'width' attribute is set along
with proportional column widths (the AsciiDoc3 docbook backend
...
default::
The default style: AsciiDoc3 inline text formatting; blank lines are
...
asciidoc3::
With this style table cells can contain any of the AsciiDoc3 elements
that are allowed inside document sections. This style runs asciidoc3
...
(the same as the AsciiDoc3 <<X65,LiteralBlock>> element).

verse::
All line breaks are retained (just like the AsciiDoc3 <<X94,verse
paragraph style>>).

[[X72]]
Markup attributes
~~~~~~~~~~~~~~~~~
AsciiDoc3 makes a number of attributes available to table markup
...
Columns containing nested tables must use the 'asciidoc3' style. An
...
better than others.  AsciiDoc3 HTML table outputs are rendered
...
AsciiDoc3 'xhtml11' backend or try a different DocBook toolchain.  Here
is a list of things to be aware of:

- Although nested tables are not legal in DocBook 4 the FOP and
  dblatex toolchains will process them correctly.  If you use `a2x3`
...
- DocBook only allows a subset of block elements inside table cells so
  not all AsciiDoc3 elements produce valid DocBook inside table cells.
  If you get validation errors running `a2x3` try the `--no-xmllint`
...
- Text formatting in cells using the 'monospaced' table style will
  ...
  is a kludge on the part of AsciiDoc3 as there is no easy way to set
  the font style in DocBook.

- Cell alignments are ignored for 'verse', 'literal' or 'asciidoc3'
  table styles.
...
can write AsciiDoc3 files that will generate HTML and PDF man pages
plus the native manpage roff format.  The easiest way to generate roff
manpages from AsciiDoc3 source is to use the a2x3 command. The
following example generates a roff formatted manpage file called
`asciidoc3.1` (a2x3 uses asciidoc3 to convert `asciidoc3.1.txt` to
DocBook which it then converts to roff using DocBook XSL Stylesheets):

  a2x3 --doctype manpage --format manpage asciidoc3.1.txt
...
brackets, for example 'ASCIIDOC3(1)'. The title name should not contain
...
element 'source', 'version' and 'manual' values using AsciiDoc3
`{mansource}`, `{manversion}` and `{manmanual}` attributes
respectively. This example is from the AsciiDoc3 header of a man page
source file:

  :man source:   AsciiDoc3
  :man version:  {revnumber}
  :man manual:   AsciiDoc3 Manual
...
in XHTML documents generated via the AsciiDoc3 'xhtml11' and 'html5' backends.
...
documents generated using the AsciiDoc3 'xhtml11' and 'html5' backends.
AsciiDoc3 uses the
...
For more examples see the {website}[AsciiDoc3 website] or the
...
mathematics. AsciiDoc3 has no macros for MathML but users familiar with
...
Configuration Files
-------------------
AsciiDoc3 source file syntax and output file markup is largely
controlled by a set of cascading, text based, configuration files.  At
runtime The AsciiDoc3 default configuration files are combined with
...
default configuration files in the AsciiDoc3 distribution in
conjunction with asciidoc3 output files. You can view configuration
file load sequence by turning on the asciidoc3 `-v` (`--verbose`)
command-line option.
...
the asciidoc3 `-a` (`--attribute`) command-line option.
...
line). Tags are used to translate AsciiDoc3 elements to backend
markup.

An AsciiDoc3 tag definition is formatted like
`<tagname>=<starttag>|<endtag>`. For example:

  emphasis=<em>|</em>

In this example asciidoc3 replaces the | character with the
emphasized text from the AsciiDoc3 input file and writes the result to
the output file.
...
configuration file or use the asciidoc3 `--attribute name!`
...
defines AsciiDoc3 quoting characters and their corresponding backend
...
quote the text.  The following examples are taken from AsciiDoc3
configuration files:
...
file or edit the global `asciidoc3.conf` configuration file:
...
AsciiDoc3 comes with three built-in Special Word types:
...
AsciiDoc3 elements.  Since the text is normally backend dependent
...
1. The directory containing the asciidoc3 executable.
2. If there is no `asciidoc3.conf` file in the directory containing the
   asciidoc3 executable then load from the global configuration
   directory (normally `/etc/asciidoc3` or `/usr/local/etc/asciidoc3`)
   i.e. the global configuration files directory is skipped if
   AsciiDoc3 configuration files are installed in the same directory as
   the asciidoc3 executable. This allows both a system wide copy and
   multiple local copies of AsciiDoc3 to coexist on the same host PC.
3. The user's `$HOME/.asciidoc3` directory (if it exists).
4. The directory containing the AsciiDoc3 source file.
5. Explicit configuration files specified using:
   - The `conf-files` attribute (one or more file names separated by a
     ...
     command-line option.
   - The asciidoc3 `--conf-file` command-line option.  The
...
- `asciidoc3.conf` from location 4.
...
- `<backend>` and `<doctype>` are values specified by the asciidoc3
  `-b` (`--backend`) and `-d` (`--doctype`) command-line options.
- `<infile>` is the path name of the AsciiDoc3 input file without the
  file name extension.
- `<lang>` is a two letter country code set by the the AsciiDoc3 'lang'
  attribute.
...
TIP: Use the asciidoc3 `-v` (`--verbose`) command-line option to see
...
and is used for textual substitution in AsciiDoc3 documents and
...
AsciiDoc3 configuration files to facilitate conditional markup
generation.
...
assigned within an AsciiDoc3 document. Attribute entries are added to
...
- The attribute entry line begins with colon -- no white space allowed
  in left margin.
- AsciiDoc3 converts the `<name>` to a legal attribute name (lower
...
AsciiDoc3 User Manual
====================
Berthold Gehrke <berthold.gehrke@gmail.com>
1.2.3, June 17, 2020
---------------------------------------------------------------------
...
- This feature can only be used in attribute entries -- configuration
  attributes cannot be set using the asciidoc3 command `--attribute`
  option.

[[X62]]
.Attribute entries promote clarity and eliminate repetition
*********************************************************************
URLs and file names in AsciiDoc3 macros are often quite long -- they
...
- If you define a an option attribute globally (for example with an
  ...
  document.
- AsciiDoc3 implements a number of predefined options which are listed
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Conditional attributes are mainly used in AsciiDoc3 configuration
...
`{set:<attrname>[!][:<value>]}`::
        ...
        <<X18,AttributeEntries>> in AsciiDoc3 documents).
...
automatically from: AsciiDoc3 document header parameters; asciidoc3
...
predefined intrinsic attributes:
  ...
  {asciidoc3-args}      used to pass inherited arguments to asciidoc3 filters
  {asciidoc3-confdir}   the asciidoc3 global configuration directory
  {asciidoc3-dir}       the asciidoc3 application directory
  {asciidoc3-file}      the full path name of the asciidoc3 script
  {asciidoc3-version}   the version of asciidoc3
  ...
  {empty}               empty string ''
  {encoding}            n.a., use {output|input-encoding} instead
  ...
  {input-encoding}      specifies input encoding
  ...
  {output-encoding}     specifies output encoding
  {plus}                plus character
  {python3}             the full path name of the Python interpreter executable
  ...
  {user-dir}            the ~/.asciidoc3 directory (if it exists)
...
3.  See also the <<X88,Backend Attributes>> section for attributes
    that relate to AsciiDoc3 XHTML file generation.
...
5. `{docfile}` and `{docdir}` refer to root document specified on the
   asciidoc3 command-line; `{infile}` and `{indir}` refer to the
...
<<X7,AsciiDoc3 configuration file>> sections.
...
AsciiDoc3 Paragraph, DelimitedBlock, List and Table block elements
...
modify the behavior of existing types by editing AsciiDoc3
configuration files.
...
tags::
  ...
  'entry', 'label', 'term', 'text') map the AsciiDoc3 list structure to
  backend markup; see the 'listtags' sections in the AsciiDoc3
...
Filters
-------
AsciiDoc3 filters allow external commands to process AsciiDoc3
...
The asciidoc3 command `--filter` option can be used to install and
...
AsciiDoc3 source document to the filter.
...
asciidoc3 recursively searches for the executable filter command:

- First it looks in the user's `$HOME/.asciidoc3/filters` directory.
- Next the global filters directory (usually `/etc/asciidoc3/filters`
  or `/usr/local/etc/asciidoc3`) directory is searched.
- Then it looks in the asciidoc3 `./filters` directory.
...
(all filters shipped with AsciiDoc3 use this technique). The filter is
...
asciidoc3 auto-loads all `.conf` files found in the filter search
...
Example Filter
~~~~~~~~~~~~~~
AsciiDoc3 comes with a toy filter for highlighting source code keywords
...
use the source-highlight-filter.

Built-in filters
~~~~~~~~~~~~~~~~
The AsciiDoc3 distribution includes 'source', 'music', 'latex' and
'graphviz' filters, details and examples are in the distribution files ./filters/*.
...
|latex
|The {website}latex-filter.html[AsciiDoc3 LaTeX filter] translates
LaTeX source to an image that is automatically inserted into the
AsciiDoc3 output documents.

|graphviz
|Gouichi Iisaka has written a http://www.graphviz.org/[Graphviz]
filter for AsciiDoc(3).  Graphviz generates diagrams from a textual
specification. Gouichi Iisaka's Graphviz filter is included in the
AsciiDoc3 distribution. AsciiDoc3 Graphviz examples live in the
 `./filters/` directory.
...
Filter plugins
~~~~~~~~~~~~~~
Filter <<X101,plugins>> are a mechanism for distributing AsciiDoc3
filters.  A filter plugin is a Zip file containing the files that
constitute a filter.  The asciidoc3 `--filter` option is used to
...
- By default filter plugins are installed in
  `$HOME/.asciidoc3/filters/<filter>` where `<filter>` is the filter
  name.
...
Plugins
-------
The AsciiDoc3 plugin architecture is an extension mechanism that allows
additional <<X100,backends>>, <<X58,filters>> and <<X99,themes>> to be
added to AsciiDoc3.

- A plugin is a Zip file containing an AsciiDoc3 backend, filter or
  theme (configuration files, stylesheets, scripts, images).
- The asciidoc3 `--backend`, `--filter` and `--theme` command-line
...
Plugin commands and conventions are documented in the asciidoc3 man
page. You can find lists of plugins at asciidoc3.org.


[[X36]]
Help Commands
-------------
The asciidoc3 command has a `--help` option which prints help topics
to stdout. The default topic summarizes asciidoc3 usage:
...
Examples:
  ...
  $ asciidoc3 -h m              # Short version of previous example.
  $ asciidoc3 --help syntax
  $ asciidoc3 -h s              # Short version of previous example.
...
When the `--help` command-line option is specified AsciiDoc3 loads the
...
Know Your Editor
~~~~~~~~~~~~~~~~
Writing AsciiDoc3 documents will be a whole lot more pleasant if you
...
[[X20]]
Vim Commands for Formatting AsciiDoc3
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Troubleshooting
~~~~~~~~~~~~~~~
AsciiDoc3 diagnostic features are detailed in the <<X82,Diagnostics
appendix>>.
...
Incorrect character encoding::
    ...
    AsciiDoc3 <<X54,encoding attribute>> ('input-encoding') for the correct character set
    (typically ISO-8859-1 (Latin-1) for European languages).

Invalid output::
    AsciiDoc3 attempts to validate the input AsciiDoc3 source but makes
    no attempt to validate the output markup, it leaves that to
    external tools such as `xmllint(1)` (integrated into `a2x3`).
    Backend validation cannot be hardcoded into AsciiDoc3 because
...
Special characters in attribute values::
    ...
    $ asciidoc3 -a 'orgname=Bill &amp; Ben Inc.' mydoc.txt
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
You have a number of stand-alone AsciiDoc3 documents that you want to
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
You have divided your AsciiDoc3 document into separate files (one per
...
This is easy because asciidoc3 will quite happily process
...
be included in another file. For example:
  ...
  $ asciidoc3 -sb docbook section1.txt

asciidoc3 can be used as a filter, so you can pipe chunks of text
through it. For example:

  $ echo 'Hello *World!*' | asciidoc3 -s -
  <div class="paragraph"><p>Hello <strong>World!</strong></p></div>

Badges in HTML page footers
~~~~~~~~~~~~~~~~~~~~~~~~~~~
See the `[footer]` section in the AsciiDoc3 distribution `xhtml11.conf`
configuration file.

Pretty printing AsciiDoc3 output
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
If the indentation and layout of the AsciiDoc3 output is not to your
liking you can:
...
2. Use Dave Raggett's http://www.html-tidy.org/[HTML Tidy] program
   to tidy asciidoc3 output. Example:

   $ asciidoc3 -b docbook -o - mydoc.txt | tidy -indent -xml >mydoc.xml
...
using the asciidoc3 `-c` (`--dump-conf`) command-line option. You
then ship this file along with the AsciiDoc3 source document plus the
`asciidoc3.py` script. The only end user requirement is that they have
...
example creates a composite HTML configuration file for `mydoc.txt`:
  ...
  $ asciidoc3 -cb xhtml11 mydoc.txt > mydoc-xhtml11.conf

Ship `mydoc.txt`, `mydoc-html.conf`, and `asciidoc3.py`. With
these three files (and a Python interpreter) the recipient can
regenerate the HMTL output:

  $ ./asciidoc3.py -eb xhtml11 mydoc.txt
...
~~~~~~~~~~~~~~~~~~~~~~~
Use `xmllint(1)` to check the AsciiDoc3 generated markup is both well
...
[TIP]
To validate DocBook 45 or 51 documents you may use the shipped Python programs; see the docstring and ./doc/readme_docbook51.txt.

----------------------
python3 ./tests/docbook_validation/asciidoc3_docbook45_validation.py my_docbook45_text.xml

python3 ./tests/docbook_validation/asciidoc3_docbook51_relaxng_validation.py my_docbook51_text.xml

python3 ./tests/docbook_validation/asciidoc3_docbook51_schematron_validation.py my_docbook51_text.xml
...
[glossary]
[[X8]] Block element::
    An AsciiDoc3 block element is a document entity composed of one or
    more whole lines of text.

[[X34]] Inline element::
    AsciiDoc3 inline elements occur within block element textual
    content, they perform formatting and substitution tasks.

Formal element::
    An AsciiDoc3 block element that has a BlockTitle. Formal elements
...
------------------
[[X53]]
Experienced users please note the following differences between AsciiDoc and AsciiDoc3:

.New in AsciiDoc3
************************************************************************
- asciidoc.conf is ignored, use asciidoc3.conf, +
- utf-8-BOM is no longer supported (will be ignored),
- deprecated quirks-modus is no longer supported,
- deprecated old-table syntax is no longer supported,
- deprecated option unsafe is no longer supported,
- there is no attribute encoding any more, use 'input-encoding' and 'output-encoding' instead,
- use only the files shipping with this distribution: xhtml11.conf v2 is *not* the same as v3,
- this applies to most of the other conf-files, too.
- ...
************************************************************************
...
AsciiDoc3 Safe Mode
-------------------
AsciiDoc3 'safe mode' skips potentially dangerous scripted sections in
AsciiDoc3 source files by inhibiting the execution of arbitrary code or
the inclusion of arbitrary files.

The safe mode is disabled by default, it can be enabled with the
asciidoc3 `--safe` command-line option.
...
[WARNING]
=====================================================================
The safe mode is not designed to protect against unsafe AsciiDoc3
...
[appendix]
Using AsciiDoc3 with non-English Languages
-----------------------------------------
AsciiDoc3 can process UTF-8 character sets but there are some things
you need to be aware of:

- If you are generating output documents using a DocBook toolchain
  then you should set the AsciiDoc3 `lang` attribute to the appropriate
  ...
  $ a2x3 -a lang=es doc/article.txt

- If you are outputting HTML directly from asciidoc3 you'll
  ...
  to create a language `.conf` file (see the AsciiDoc3's `lang-en.conf`
  file).
+
NOTE: You still use the 'NOTE', 'CAUTION', 'TIP', 'WARNING',
'IMPORTANT' captions in the AsciiDoc3 source, they get translated in
the HTML output file.

- asciidoc3 automatically loads configuration files named like
  `lang-<lang>.conf` where `<lang>` is a two letter language code that
  matches the current AsciiDoc3 `lang` attribute. See also
  <<X27,Configuration File Names and Locations>>.

- If you miss the file for your favorite language, search the web for
  something like 'lang conf asciidoc <your two letter-lang-code like
  en or fr here>'. Chances are that anybody else has already had the
  same question and provides a conf-file. Or you write it by yourself
  (and please, send it to asciidoc3.org)!
...
reading AsciiDoc3 documents much easier syntax highlighting also helps
you catch AsciiDoc3 syntax errors as you write your documents.

The AsciiDoc3 distribution directory contains a Vim syntax highlighter
for AsciiDoc3 (`./vim/syntax/asciidoc.vim`).

Install the highlighter by copying `asciidoc3.vim` to your
...
- or execute the Vim command `:set syntax=asciidoc3`.
- or add the following line to the end of you AsciiDoc3 source files:

  // vim: set syntax=asciidoc3:

The highlighter is (almost) the same as in AsciiDoc v2 - but should 
work though. Thanks al lot for all contributors!
...
|====================================================================
|Option|Backends|AsciiDoc3 Elements|Description

|autowidth |xhtml11, html5, html4 |table|
The column widths are determined by the browser, not the AsciiDoc3
...
|compact |docbook45/51, xhtml11, html5 |bulleted list, numbered list|
Minimizes vertical space in the list

|footer |docbook45/51, xhtml11, html5, html4 |table|
The last row of the table is rendered as a footer.

|header |docbook45/51, xhtml11, html5, html4 |table|
The first row of the table is rendered as a header.

|pgwide |docbook45/51 (XSL/FO) |table, block image, horizontal labeled list|
...
Diagnostics
-----------
The `asciidoc3` `--verbose` command-line option prints additional
...
- 'trace names' are normally the names of AsciiDoc3 elements (see the
  list below).
...
. Trace the entire document.
  ...
  $ asciidoc3 -a trace mydoc.txt

. Trace messages whose names start with `quotes` or `macros`:

  $ asciidoc3 -a 'trace=quotes|macros'  mydoc.txt

. Print the first line of each trace message:

  $ asciidoc3 -a trace mydoc.txt 2>&1 | grep ^TRACE:
...
If the `disable-javascript` attribute is defined the `asciidoc3.js`
JavaScript is not embedded or linked to the output document.  By
default AsciiDoc3 automatically embeds or links the `asciidoc3.js`
...
Where `<filename>` is the file name (sans extension) of the AsciiDoc3
...
the location of the icons installed in the AsciiDoc3 configuration
directory.

|input-encoding |html4, html5, xhtml11, docbook |
Encoding of the input source, defaults to utf-8.
...
not specified the AsciiDoc3 document title is used.
...
|output-encoding |html4, html5, xhtml11, docbook |
Encoding of the output source, defaults to utf-8.
...
|quirks |xhtml11 |
Repealed, quirks-modus is no longer supported.
...
|sgml |docbook45 |
(Repealed) The `--backend=docbook45` command-line option produces DocBook 4.5
...
- The following example generates a numbered table of contents using a
  ...
  $ asciidoc3 -a toc -a numbered mydoc.txt
...
When set to 'auto' (the default value) asciidoc3 will place the
...
used with the 'toc' attribute. Example usage:
  ...
  $ asciidoc3 -a toc -a toclevels=3 doc/asciidoc3.txt
...
asciidoc3 - converts an AsciiDoc3 text file to HTML or DocBook45
or DocBook51
...
*-b, --backend*='BACKEND'::
    Backend output file format: 'docbook45', 'docbook51', 'xhtml11',
    'html4', 'html5', 'slidy', or 'latex' (the 'latex' backend is
    ...
    backend plugins (see <<Xpc,*PLUGIN COMMANDS*>>).
...
contributed to it. Porting to Python3 (AsciiDoc3) and additional work
was done by Berthold Gehrke berthold.gehrke@gmail.com


*RESOURCES*

Main web site: <https://asciidoc3.org/>
...
License
-------
(AsciiDoc Copyright (C) 2002-2011 Stuart Rackham; using Python2.x)

AsciiDoc3 Text based document generation using Python 3.x. +
